...
export default function AlumnoDashboard() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Mi Panel</h1>
        <p className="text-gray-600 mt-1 text-sm lg:text-base">Bienvenido a tu espacio de aprendizaje</p>
      </div>
        ...
        {quickActions.map((action, index) => (
          ...
          >
            <Link href={action.href}>
              <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6 border border-gray-100 hover:shadow-lg hover:border-blue-200 transition-all cursor-pointer group">
                <div className="flex items-start gap-3 lg:gap-4">
                  <div className="p-2 lg:p-3 bg-blue-50 rounded-lg group-hover:bg-blue-100 transition-colors">
                    <action.icon className="w-5 h-5 lg:w-6 lg:h-6 text-blue-500" />
                  </div>
                  <div className="flex-1">
                    <h3 className="text-base lg:text-lg font-semibold text-gray-900 group-hover:text-blue-600 transition-colors">
                      {action.title}
                    </h3>
                    <p className="text-xs lg:text-sm text-gray-600 mt-1">{action.description}</p>
                  </div>
                </div>
              </div>
            </Link>
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 lg:p-6 border border-gray-100">
        <h2 className="text-lg lg:text-xl font-semibold text-gray-900 mb-4">Próximas Clases</h2>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-8 lg:py-12">
            <Calendar className="w-10 h-10 lg:w-12 lg:h-12 text-gray-300 mx-auto mb-4" />
            <p className="text-gray-500 text-sm lg:text-base">No tienes clases programadas</p>
            <Link href={ROUTES.ALUMNO.RESERVAR}>
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="mt-4 px-4 lg:px-6 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors text-sm lg:text-base"
              >
          ...
          <div className="space-y-3">
            {upcomingClasses.map((booking) => (
              <div
                key={booking.id}
                className="flex flex-col sm:flex-row sm:items-center justify-between p-3 lg:p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors gap-3"
              >
                <div className="flex items-center gap-3 lg:gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs lg:text-sm text-gray-600">
                      {format(parseBookingDateTime(booking.date, booking.startTime), "EEE d 'de' MMM", { locale: es })}
                    </p>
                  </div>
                </div>
